...
func TestContainsFilterTrue(t *testing.T) {
	f, _ := ContainsFilter("hell")
	res := f([]byte("hi hello"))
	if res != true {
		t.Fatal("result shoult be true for `true` script")
	}
...
func TestContainsFilterFalse(t *testing.T) {
	f, _ := ContainsFilter(`nope`)
	res := f([]byte("hi hello"))
	if res != false {
		t.Fatal("result shoult be false for `false` script")
	}
...
func TestContainsFilterEvenOdd(t *testing.T) {
	f, err := ContainsFilter("bcde")
	if err != nil {
		t.Fatal(err)
	}
